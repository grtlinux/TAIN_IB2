/*
 * PROGRAM ID   : log_zip.c
 * PROGRAM NAME : log_zip 모듈
 * AUTHOR       : (주)ICA
 * COMMENT      : log_zip 모듈
...
LOG_ZIP      stLogZip            [MAX_LOG_ZIP];          /* Log Zip 정보 저장 */
int          iMaxLogZip                       ;
...
/*----------------------------------------------------------------------------*/
{
	...
	if (flag)
	{
		...
		char* p = CfgGetMst("ib.cfg.file.logzip");
		if (p == NULL)
		{
			LOG(_FL_, 0, 0, "ERROR : log_zip 환경파일 미정의");
			if (flag) exit(-1);
		...
		while (1)
		{
			...
			if (flag)
			{
				if (strcmp("EXE_PROGRAM", UtilTrim(arr[0])) == 0)
				{
					if (!flag) LOG(_FL_, 0, 0, " [EXE_PROGRAM:%s]", UtilTrim(arr[1]));
					
					strcpy(sExeProg, UtilTrim(arr[1]));
				}
				...
				else if (strcmp("OBJECT_FOLDER", UtilTrim(arr[0])) == 0)
				{
					...
					strcpy(stLogZip[iMaxLogZip].sObjPath, UtilTrim(arr1[0]));
					stLogZip[iMaxLogZip].iExpNo = UtilToInt(UtilTrim(arr1[1]));
					strcpy(stLogZip[iMaxLogZip].sPattern, UtilTrim(arr1[2]));
		...
		if (flag)
		{
			...
			LOG(_FL_, 0, 0, " sExeProg = [%s]", sExeProg);
			LOG(_FL_, 0, 0, " iExpNo   = [%d]", iExpNo);

			int i;
			for (i=0; i < iMaxLogZip; i++)
			{
				LOG(_FL_, 0, 0, "%4d/%d)", (i+1), iMaxLogZip);
				LOG(_FL_, 0, 0, "\t stLogZip[%d].sObjPath = [%s]", i, stLogZip[i].sObjPath);
				LOG(_FL_, 0, 0, "\t stLogZip[%d].iExpNo   = [%d]", i, stLogZip[i].iExpNo  );
				LOG(_FL_, 0, 0, "\t stLogZip[%d].sPattern = [%s]", i, stLogZip[i].sPattern);
			}
		}
	}
...
/*----------------------------------------------------------------------------*/
{
	...
	if (flag)
	{
		...
		if (flag)
		{
			if (flag) LOG(_FL_, 0, 0, "[%04d/%02d/%02d] 이전 폴더를 압축합니다."
				, t.tm_year + 1900
				, t.tm_mon + 1
				, t.tm_mday);
		}
...
/*----------------------------------------------------------------------------*/
{
	...
	if (flag)
	{
		...
		while ( TRUE )
		{
			...
			/* .gz 확장자 파일은 걸러낸다. */
			if (strstr(pDirEnt->d_name, ".gz") != NULL)
				continue;
			...
			{
				char sCmd[512];
				sprintf(sCmd, "%s %s", sExeProg, sFile);
				system(sCmd);
				
				if (flag) LOG(_FL_, 0, 0, "파일 압축 명령 실행 => [%s]", sCmd);
			}
		}
...
/*----------------------------------------------------------------------------*/
{
	...
	if (flag)
	{
		...
		{
			/* Expire Time을 구한다. */
			if (flag) GetCompTime(stLogZip[i].iExpNo);

			/* Expire Time에 해당하는 폴더를 처리한다. */
			if (flag) JobProc(stLogZip[i].sObjPath);
		}
	}
...
/*----------------------------------------------------------------------------*/
{
	...
	if (flag)
	{
		 ...
		 * 임시로 사용함.
		 */
		if (!flag && argc == 1 && strcmp(argv[0], "log_zip") == 0)
		{
			printf("log_zip 실행하지 말아 주세요.\n");
			exit(0);
		}
	...
	if (flag)
	{
		...
		if (argc < 2 || strcmp("LOGZIP0000", argv[1]) != 0)
		{
			/*
			 * USAGE
			 */
			printf("ERROR : Invalid argument... (Usage : %s LOGZIP0000)\n", sProgName);
			exit(0);
		}
