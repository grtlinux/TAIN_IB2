/*
 * PROGRAM ID   : ofrcom2.c
 * PROGRAM NAME : ofrcom2 관련 모듈
 * AUTHOR       : (주)ICA
 * COMMENT      : ofrcom2 관련 모듈
...
/*----------------------------------------------------------------------------*/
{
	...
	if (flag)
	{
		*sDataLine = 0x00;
        ...
        if (flag) LOG(_FL_, 0, 0, "[FQ_READ]  -> [%04d:%s] ", iDataLen, sDataLine);
...
/*----------------------------------------------------------------------------*/
{
	...
	if (flag)
	{
		...
		if (flag)
		{
			...
			if (flag) LOG(_FL_, 0, 0, "[%s] [%04d:%s][%d] ", sFepid, iDataLen, sDataLine, iFQidx);
		}
		...
		if (flag)
		{
			...
			if (strcmp(sFepid, "KFT02") == 0)
			{
				...
				int iSize = sizeof(sSvcId) / sizeof(sSvcId[0]);

				int i, j, diff, iMin = 99999;
				for (i=0, j=iFQidx; i < iSize; i++)
				{
					j = (++j % iSize);
					diff = FQ2DiffSeq(RECV_FQ, sSvcId[j]);
					if (diff < iMin)
					{
						iFQidx = j;
						iMin = diff;
				...
				FQ2* pFq = FQ2Open(RECV_FQ, sSvcId[iFQidx]);
				FQ2Write(pFq, sDataLine);
				FQ2Close(pFq);
				
				if (flag) LOG(_FL_, 0, 0, "[%s] [%04d:%s] ", sSvcId[iFQidx], iDataLen, sDataLine);
			}
	...
	if (flag)
	{
		...
		if (flag)
		{
			...
			&&  csi->iPollFlag  == FALSE)
			{
				...
				{
					/* POLL REQ 출력 */
					if (flag) LOG(_FL_, 0, 0, "[POLL_SEND:%d] REQ -> [%d:%s] (%s:%d)(fd:%d)"
	...
	if (flag)
	{
		...
		if (flag)
		{
			...
			if (Poll2CmpRecvReq(sRecvLine) == 0)
			{
				...
				{
					/* 수신 POLL REQ 출력 */
					if (flag) LOG(_FL_, 0, 0, "[POLL_RECV] <- REQ [%d:%s] (%s:%d)"
				...
				{
					/* 송신 POLL RES 출력 */
					if (flag) LOG(_FL_, 0, 0, "[POLL_SEND] RES -> [%d:%s] (%s:%d)"
