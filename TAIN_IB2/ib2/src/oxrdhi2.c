/*
 * PROGRAM ID   : oxrdhi2.c
 * PROGRAM NAME : oxrdhi2 모듈
 * AUTHOR       : (주)ICA
 * COMMENT      : oxrdhi2 모듈
 *                대외기관 연결 모듈(수신)
...
/*----------------------------------------------------------------------------*/
{
	...
	if (flag)
	{
		if (flag)
		{
			...
			if (strcmp(sFepid, "DHI02") == 0)
			{
				/*
				 * DATE 2013.10.18 : 대한생명에 적용
				 * 암호화된 Password를 복호화하여 수신전문을 만든다.
				 */
				if (!flag) LOG(_FL_, 0, 0, "[%s] process MSockMsgRecv_DecPass..", sFepid);
				iRecvLen = MSockMsgRecv_DecPass(csi->sock, sRecvLine);
			}
			else
			{
				/*
				 * 암호화가 적용되지 않은 전문을 수신처리한다.
				 */
