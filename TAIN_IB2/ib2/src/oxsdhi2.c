/*
 * PROGRAM ID   : oxsdhi2.c
 * PROGRAM NAME : oxsdhi2 모듈
 * AUTHOR       : (주)ICA
 * COMMENT      : oxsdhi2 모듈
 *                대외기관 연결 모듈 (송신)
	...
	if (flag)
	{
		...
		if (flag)
		{
			...
			&&  csi->iPollFlag  == FALSE)
			{
				...
				{
					/*
					 * 대한생명 이외의 기관에서는 POLL요청송신하고 POLL응답수신한다.
					 * 그래서 아래와 같이 세팅하면 된다.
					 */
					csi->iPollFlag = TRUE;
				}
				...
				{
					/*
					 * DATE 2013.10.18 : 대한생명 적용용
					 * 대한생명 기관에서는 POLL요청송신하면 POLL응답수신은 수신프로세스가
					 * 담당한다. 그래서 이 송신프로세스는 단순히 POLL 송신만 담당한다.
					 * 그래서 아래와 같이 특정 시간을 주기로 POLL을 송신한다.
					 */
					csi->iPollFlag = FALSE;
