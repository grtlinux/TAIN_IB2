...
typedef MSG_KFT1_HDR  MSG_HDR ;
typedef MSG_KFT1_LINE MSG_LINE;
typedef MSG_KFT1_0600 MSG_MANG;
typedef MSG_KFT1_0630 MSG_FILE;
typedef MSG_KFT1_0310 MSG_DATA;
typedef MSG_KFT1_0620 MSG_MREQ;
typedef MSG_KFT1_0300 MSG_MRES;
...
char            sLine           [MAX_DATA_LINE];   /* 데이터부 짜투리포함 */
int             iLen            ;
	...
	if (flag)
	{
		...
		if (flag)
		{
			...
			sprintf(sSendEdiLine, "%04d%3.3s%3.3s%4.4s%1.1s%1.1s%-8.8s%3.3s%s"
				, iSendEdiLen - 4
				, CfgGetBxa("BIZ_CODE")  /* "KTB" */
				, CfgGetBxa("ORG_CODE")  /* "269" */
				, sMsgCls
				, "S"
				, "B"
				, sFileId
		...
		if (flag)
		{
			...
			if (flag)
			{
				...
				if (strcmp("000", sResCode) != 0)
				{
					...
					{
						LOG(_FL_, 0, 0, "ERROR : 시스템 장애(비밀번호확인) = '%s'", sResCode);
					}
					...
					{
						LOG(_FL_, 0, 0, "ERROR : 송신자암호 오류 = '%s'", sResCode);
					}
				    ...
				    else if (strcmp("634", sResCode) == 0)
				    {
					    LOG(_FL_, 0, 0, "ERROR : 결제원 수신 거부(업무확인) = '%s'", sResCode);
				    }
...
/*----------------------------------------------------------------------------*/
{
	...
	if (flag)
	{
		if (flag)
		{
			...
			sprintf(sDataLine, "%-8.8s%012d%4.4s"
				, pFile->sFileId
				, 0
				, pFile->sMsgByte);
			 ...
			 */
			sprintf(sFileId, "%-8.8s", pFile->sFileId);
		}
...
/*----------------------------------------------------------------------------*/
{
	...
	if (flag)
	{
		...
		if (flag)
		{
			...
			strcpy(sLine + iLen, pData->sMsgDtl);
			...
			int i;
			char* p;
			for (i=0, p=sLine; i < iCnt; i++, p += iRecLen)
			{
				fprintf(fp, "%*.*s\n", iRecLen, iRecLen, p);
				iRecCnt ++;
				iLen -= iRecLen;
			...
			if (flag)
			{
				 ...
				 * 짜투리를 앞으로 이동한다.
				 */
				UtilMemMove(sLine, sLine + (iCnt * iRecLen), iLen);
			}
		}
	}
	...
	if (flag)
	{
		...
		switch (-1)
		{
			...
			default:
				if (flag)
				{
					sprintf(sDataLine, "%04d%03d%03d%*.*d"
						, iBlockNo
						, iSeqNo
						, 0
						, (iSeqNo * 1),(iSeqNo * 1),1
		...
		*sFileId = 0x00;
		...
		if (flag)
		{
			if (flagFQ == FALSE)
			{
				...
				if (flag) LOG(_FL_, 0, 0, "[RecvBatFileDONE] : sBatInf [%s]", sBatInf);
...
/*----------------------------------------------------------------------------*/
{
	...
	if (flag)
	{
		...
		if (flag) LOG(_FL_, 0, 0, "---- BAT_INF* pInf --------------------------------");
		if (flag) LOG(_FL_, 0, 0, "[%s]", sBatInf);
		if (flag) LOG(_FL_, 0, 0, "pInf->sRecLen    = [%*.*s]", sizeof(pInf->sRecLen   ), sizeof(pInf->sRecLen   ), pInf->sRecLen   );
		if (flag) LOG(_FL_, 0, 0, "pInf->sRecCnt    = [%*.*s]", sizeof(pInf->sRecCnt   ), sizeof(pInf->sRecCnt   ), pInf->sRecCnt   );
		if (flag) LOG(_FL_, 0, 0, "pInf->sSR        = [%*.*s]", sizeof(pInf->sSR       ), sizeof(pInf->sSR       ), pInf->sSR       );
		if (flag) LOG(_FL_, 0, 0, "pInf->sOrgCode   = [%*.*s]", sizeof(pInf->sOrgCode  ), sizeof(pInf->sOrgCode  ), pInf->sOrgCode  );
		if (flag) LOG(_FL_, 0, 0, "pInf->sTrCode    = [%*.*s]", sizeof(pInf->sTrCode   ), sizeof(pInf->sTrCode   ), pInf->sTrCode   );
		if (flag) LOG(_FL_, 0, 0, "pInf->sTrCodeLen = [%*.*s]", sizeof(pInf->sTrCodeLen), sizeof(pInf->sTrCodeLen), pInf->sTrCodeLen);
		if (flag) LOG(_FL_, 0, 0, "pInf->sDate      = [%*.*s]", sizeof(pInf->sDate     ), sizeof(pInf->sDate     ), pInf->sDate     );
		if (flag) LOG(_FL_, 0, 0, "pInf->sTime      = [%*.*s]", sizeof(pInf->sTime     ), sizeof(pInf->sTime     ), pInf->sTime     );
		if (flag) LOG(_FL_, 0, 0, "pInf->sTrCodeSeq = [%*.*s]", sizeof(pInf->sTrCodeSeq), sizeof(pInf->sTrCodeSeq), pInf->sTrCodeSeq);
		if (flag) LOG(_FL_, 0, 0, "pInf->sFiller    = [%*.*s]", sizeof(pInf->sFiller   ), sizeof(pInf->sFiller   ), pInf->sFiller   );
		if (flag) LOG(_FL_, 0, 0, "---------------------------------------------------");
	}

	if (flag)
	{
		if (flag) LOG(_FL_, 0, 0, "---- bxa_trlst.cfg --------------------------------");
		if (flag) LOG(_FL_, 0, 0, "SERVICE = [%s]", CfgGetBxaTr("SERVICE"));
		if (flag) LOG(_FL_, 0, 0, "FEPID   = [%s]", CfgGetBxaTr("FEPID"  ));
		if (flag) LOG(_FL_, 0, 0, "ORGCD   = [%s]", CfgGetBxaTr("ORGCD"  ));
		if (flag) LOG(_FL_, 0, 0, "SNDRCV  = [%s]", CfgGetBxaTr("SNDRCV" ));
		if (flag) LOG(_FL_, 0, 0, "FILEID  = [%s]", CfgGetBxaTr("FILEID" ));
		if (flag) LOG(_FL_, 0, 0, "TRCODE  = [%s]", CfgGetBxaTr("TRCODE" ));
		if (flag) LOG(_FL_, 0, 0, "TRCDLEN = [%s]", CfgGetBxaTr("TRCDLEN"));
		if (flag) LOG(_FL_, 0, 0, "RECLEN  = [%s]", CfgGetBxaTr("RECLEN" ));
		if (flag) LOG(_FL_, 0, 0, "COMMENT = [%s]", CfgGetBxaTr("COMMENT"));
		if (flag) LOG(_FL_, 0, 0, "---------------------------------------------------");
	}
}
...
/*----------------------------------------------------------------------------*/
{
	...
	if (!flag && iForkFlag)  /* DATE.2013.10.28 : 배치송수신 수정을 위해 fork를 제거함 */
	{
		/*
	...
	if (flag)
	{
		...
		while (TRUE)
		{
			...
			if (flag)
			{
				...
				{
					if (flag) LOG(_FL_, 0, 0, "수신파일없음 -> 세션 종료");
			        /* 세션을 종료한다. */
			        CSockFDCLR_All();
                    if (!flag) sleep(60);
                    if (!flag) continue;
