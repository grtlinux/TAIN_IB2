/*
 * PROGRAM ID   : ofscho2.c
 * PROGRAM NAME : ofscho2 관련 모듈
 * AUTHOR       : (주)ICA
 * COMMENT      : ofscho2 관련 모듈
	...
	if (flag)
	{
		*sDataLine = 0x00;
		...
		if (flag)
		{
			 ...
			 */
			FQ3* pFq = FQ3Open(SEND_FQ, sServiceid);
...
/*----------------------------------------------------------------------------*/
{
	...
	if (flag)
	{
		...
		if (flag)
		{
			 ...
			 */
			FQ3* pFq = FQ3Open(RECV_FQ, sServiceid);

			FQ3Write(pFq, sDataLine);
...
/*----------------------------------------------------------------------------*/
{
	...
	if (flag)
	{
		...
		{
			/* 사용할 FQ를 선택한다. */
			if (flag)  FQ3Init(SEND_FQ, sServiceid);
			if (!flag) FQ3Init(RECV_FQ, sServiceid);
		}
