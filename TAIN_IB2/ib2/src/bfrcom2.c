/*
 * PROGRAM ID   : bfrcom2.c
 * PROGRAM NAME : bfrcom2 모듈
 * AUTHOR       : (주)ICA
 * COMMENT      : bfrcom2 모듈
...
/*----------------------------------------------------------------------------*/
{
	...
	if (flag)
	{
		if (flag)
		{
			 ...
			 * 파일명을 만든다.
			 */
			sprintf(sFileName, CfgGetMaster("ib.path.file.bfrcom")
				, sizeof(pBat->sSR       ), sizeof(pBat->sSR       ), pBat->sSR
				, sizeof(pBat->sOrgCode  ), sizeof(pBat->sOrgCode  ), pBat->sOrgCode
				, sizeof(pBat->sTrCode   ), sizeof(pBat->sTrCode   ), pBat->sTrCode
				, sizeof(pBat->sDate     ), sizeof(pBat->sDate     ), pBat->sDate
		...
		if (flag)
		{
			...
			chmod(sFileName, S_IRUSR|S_IWUSR | S_IRGRP|S_IWGRP | S_IROTH|S_IWOTH);
...
/*----------------------------------------------------------------------------*/
{
	...
	if (flag)
	{
		...
		FQ2* pFq = FQ2Open("OFR", sServiceid);
