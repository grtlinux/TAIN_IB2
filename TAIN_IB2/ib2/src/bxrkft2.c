...
/*----------------------------------------------------------------------------*/
{
	...
	if (flag)
	{
		...
		if (flag)
		{
			...
			/*sprintf(sTrCode, "%4.4s0000%3.3s", pFile->sFileId, CfgGetBxa("ORG_CODE") + 5);*/
			sprintf(sTrCode, "%4.4s0000%3.3s", pFile->sFileId, CfgGetBxa("ORG_CODE"));

			if(flag) printf("[%s:%d] [sFileId:%8.8s][OrgCode:%s][sTrCode:%s]\n"
				, __FUNCTION__, __LINE__
				, pFile->sFileId, CfgGetBxa("ORG_CODE"), sTrCode);
...
/*----------------------------------------------------------------------------*/
{
	...
	if (flag)
	{
		...
		if (flag) LOG(_FL_, 0, 0, "pInf->sRecLen    = [%*.*s]\n", sizeof(pInf->sRecLen   ), sizeof(pInf->sRecLen   ), pInf->sRecLen   );
		if (flag) LOG(_FL_, 0, 0, "pInf->sRecCnt    = [%*.*s]\n", sizeof(pInf->sRecCnt   ), sizeof(pInf->sRecCnt   ), pInf->sRecCnt   );
		if (flag) LOG(_FL_, 0, 0, "pInf->sSR        = [%*.*s]\n", sizeof(pInf->sSR       ), sizeof(pInf->sSR       ), pInf->sSR       );
		if (flag) LOG(_FL_, 0, 0, "pInf->sOrgCode   = [%*.*s]\n", sizeof(pInf->sOrgCode  ), sizeof(pInf->sOrgCode  ), pInf->sOrgCode  );
		if (flag) LOG(_FL_, 0, 0, "pInf->sTrCode    = [%*.*s]\n", sizeof(pInf->sTrCode   ), sizeof(pInf->sTrCode   ), pInf->sTrCode   );
		if (flag) LOG(_FL_, 0, 0, "pInf->sTrCodeLen = [%*.*s]\n", sizeof(pInf->sTrCodeLen), sizeof(pInf->sTrCodeLen), pInf->sTrCodeLen);
		if (flag) LOG(_FL_, 0, 0, "pInf->sDate      = [%*.*s]\n", sizeof(pInf->sDate     ), sizeof(pInf->sDate     ), pInf->sDate     );
		if (flag) LOG(_FL_, 0, 0, "pInf->sTime      = [%*.*s]\n", sizeof(pInf->sTime     ), sizeof(pInf->sTime     ), pInf->sTime     );
		if (flag) LOG(_FL_, 0, 0, "pInf->sTrCodeSeq = [%*.*s]\n", sizeof(pInf->sTrCodeSeq), sizeof(pInf->sTrCodeSeq), pInf->sTrCodeSeq);
		if (flag) LOG(_FL_, 0, 0, "pInf->sFiller    = [%*.*s]\n", sizeof(pInf->sFiller   ), sizeof(pInf->sFiller   ), pInf->sFiller   );
	...
	if (flag)
	{
		...
		if (flag) LOG(_FL_, 0, 0, "SERVICE = [%s]\n", CfgGetBxaTr("SERVICE"));
		if (flag) LOG(_FL_, 0, 0, "FEPID   = [%s]\n", CfgGetBxaTr("FEPID"  ));
		if (flag) LOG(_FL_, 0, 0, "ORGCD   = [%s]\n", CfgGetBxaTr("ORGCD"  ));
		if (flag) LOG(_FL_, 0, 0, "SNDRCV  = [%s]\n", CfgGetBxaTr("SNDRCV" ));
		if (flag) LOG(_FL_, 0, 0, "FILEID  = [%s]\n", CfgGetBxaTr("FILEID" ));
		if (flag) LOG(_FL_, 0, 0, "TRCODE  = [%s]\n", CfgGetBxaTr("TRCODE" ));
		if (flag) LOG(_FL_, 0, 0, "TRCDLEN = [%s]\n", CfgGetBxaTr("TRCDLEN"));
		if (flag) LOG(_FL_, 0, 0, "RECLEN  = [%s]\n", CfgGetBxaTr("RECLEN" ));
		if (flag) LOG(_FL_, 0, 0, "COMMENT = [%s]\n", CfgGetBxaTr("COMMENT"));
		if (flag) LOG(_FL_, 0, 0, "---------------------------------------------------");
	}
}
...
/*----------------------------------------------------------------------------*/
{
	...
	if (flag)
	{
		...
		if (!flag)
		{
			...
			printf("CfgGetBxa    BIZ_CODE  =[%s]\n", CfgGetBxa("BIZ_CODE"));
			printf("CfgGetBxa    ORG_ID    =[%s]\n", CfgGetBxa("ORG_CODE"));
			printf("CfgGetBxa    SENDER_ID =[%s]\n", sUserid);
			printf("CfgGetBxa    SENDER_PWD=[%s]\n", sPasswd);
		}

		strcpy(sUserid, CfgGetBxa("SENDER_ID" ));
		strcpy(sPasswd, GetPassword("KFT52", 1, sUserid, CfgGetBxa("SENDER_PWD"), CfgGetBxa("ORG_CODE")));
		...
		if (!flag)
		{
			...
			strcpy(sEnc, GetPassword("KFT52", 1, sUserid, sOrg, CfgGetBxa("ORG_CODE")));
			printf("OUTPUT 인코딩암호=[%s]\n", sEnc );

			/*
			 *  복호화
			 */
			strcpy(sDec, GetPassword("KFT52", 2, sUserid, sEnc, CfgGetBxa("ORG_CODE")));
