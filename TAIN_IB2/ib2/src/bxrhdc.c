...
typedef MSG_HDC_HDR    MSG_HDR ;
typedef MSG_HDC_LINE   MSG_LINE;
typedef MSG_HDC_0600   MSG_MANG;
typedef MSG_HDC_0630   MSG_FILE;
typedef MSG_HDC_0310   MSG_DATA;
typedef MSG_HDC_0620   MSG_MREQ;
typedef MSG_HDC_0300   MSG_MRES;
...
/*----------------------------------------------------------------------------*/
{
	...
	if (flag)
	{
		...
		printf("\tp->sFileId = [%*.*s]\n", sizeof(p->sFileId), sizeof(p->sFileId), p->sFileId);
		printf("\tp->sCdDate = [%*.*s]\n", sizeof(p->sCdDate), sizeof(p->sCdDate), p->sCdDate);
...
/*----------------------------------------------------------------------------*/
{
	...
	if (flag)
	{
		...
		if (flag)
		{
            ...
            sprintf(sSendEdiLine, "%3.3s%-8.8s%4.4s%1.1s%1.1s%-4.4s%-4.4s%3.3s%s"
                , "FTE"
                , CfgGetBxa("ORG_CODE")   /* 'HYUNDAI ' */
                , sMsgCls
                , "R"
                , "E"
                , sFileId
		...
		if (flag)
		{
			 ...
			 */
			int ret = X25MsgSendFFEF(cSock, sSendEdiLine, iSendEdiLen);
			if (ret <= 0)
			{
				CSockFDCLR(cSock);
				return FALSE;
	...
	if (flag)
	{
		...
		if (flag)
		{
			...
			if (flag)
			{
				 ...
				 * 전문에서 길이부분을 읽는다.
				 */
				iRecvEdiLen = X25MsgRecvFFEF(cSock, sRecvEdiLine);
				if (iRecvEdiLen <= 0)
				{
					CSockFDCLR(cSock);
					return FALSE;
...
/*----------------------------------------------------------------------------*/
{
	...
	if (flag)
	{
		if (flag)
		{
			 ...
			 */
			sprintf(sFileId, "%-4.4s", pFile->sFileId);
		}
		...
		if (flag)
		{
			...
			sprintf(sTrCode, "%4.4s0000000", pFile->sFileId);

			if(flag) printf("[%s:%d] [sFileId:%-4.4s][sCdDate:%-4.4s][OrgCode:%s][sTrCode:%s]\n"
				, __FUNCTION__, __LINE__
				, pFile->sFileId, pFile->sCdDate, CfgGetBxa("ORG_CODE"), sTrCode);
	...
	if (flag)
	{
		...
		if (flag)
		{
			if (flagFQ == FALSE)
			{
				...
				if(flag) printf("[%s:%d] [sFileId:%4.4s][OrgCode:%s][sTrCode:%s]\n"
					, __FUNCTION__, __LINE__
...
/*----------------------------------------------------------------------------*/
{
	...
	if (flag) LOG(_FL_, 0, 0,  "[RecvBatFileEDI] START");
	...
	if (flag) LOG(_FL_, 0, 0,  "[RecvBatFileEDI] END");
