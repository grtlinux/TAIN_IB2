...
#define SeedRound(L0, L1, R0, R1, K) {             \
	...
	T1 = R1 ^ (K)[1];                              \
	T1 ^= T0;                                      \
	T1 = SSS0[GetB0(T1)] ^ SSS1[GetB1(T1)] ^         \
		 SSS2[GetB2(T1)] ^ SSS3[GetB3(T1)];          \
	T0 += T1;                                      \
	T0 = SSS0[GetB0(T0)] ^ SSS1[GetB1(T0)] ^         \
		 SSS2[GetB2(T0)] ^ SSS3[GetB3(T0)];          \
	T1 += T0;                                      \
	T1 = SSS0[GetB0(T1)] ^ SSS1[GetB1(T1)] ^         \
		 SSS2[GetB2(T1)] ^ SSS3[GetB3(T1)];          \
	T0 += T1;                                      \
...
/*============================================================================*/
/*
 * 내용 : same as encrypt, except that round keys are applied in reverse order.
 */
/*----------------------------------------------------------------------------*/
...
***/
#define KC0     0x9eff79b9UL
#define KC1     0x3c6ecd73UL
#define KC2     0xe8dde6e6UL
#define KC3     0xf15234ccUL
#define KC4     0xeaab9b99UL
#define KC5     0xc8ef1833UL
#define KC6     0x234e6e67UL
#define KC7     0x1bbcbdbfUL
#define KC8     0xabc9b99eUL
#define KC9     0x6efef33cUL
#define KC10    0x1818e678UL
#define KC11    0xbbc245f1UL
#if NoRounds == 16
#define KC12    0xa89899e3UL
#define KC13    0xef6638c6UL
#define KC14    0xde62365dUL
#define KC15    0xbdd11f1bUL
#endif
...
#define EncRoundKeyUpdate0(K, A, B, C, D, KC) {  \
	...
	T0 = A + C - KC;                             \
	T1 = B + KC - D;                             \
	(K)[0] = SSS0[GetB0(T0)] ^ SSS1[GetB1(T0)] ^   \
			 SSS2[GetB2(T0)] ^ SSS3[GetB3(T0)];    \
	(K)[1] = SSS0[GetB0(T1)] ^ SSS1[GetB1(T1)] ^   \
			 SSS2[GetB2(T1)] ^ SSS3[GetB3(T1)];    \
}
...
*/
	...
	{
		for(i = 0; i < 16; i++) {
			sprintf(temp + i * 5, "0x%02X ", session_key[i]);
		}
	...
	if (1)
	{
		...
		for(i = 0; i < 16; i++) {
			sprintf(temp + i * 5, "0x%02X ", encPassword[i]);
		}
		printf("암호화  [%s]\n", temp);
	}
	...
	if (1)
	{
		...
		unsigned char encPass[16] = {
			0x3A, 0x72, 0x4B, 0x57, 0x67, 0x39, 0x8E, 0xF8, 0x45, 0x1B, 0x2C, 0x3F, 0x9A, 0x56, 0x51, 0x20
		};
