/*
 * PROGRAM ID   : bfscom2.c
 * PROGRAM NAME : bfscom2 모듈
 * AUTHOR       : (주)ICA
 * COMMENT      : bfscom2 모듈
 * HISTORY      : 2013.10.09 강석. 최초작성
 *
 * FEP 배치파일 송신
 *
 *  1. 배치FQ를 읽는다.
...
/*
 * 내용 : 순차적인 값을 리턴한다.
 * DATE.2013.09.27 : AP#2 서버의 FQ (FEPID02)연결을 위한 작업
...
/*----------------------------------------------------------------------------*/
{
	...
	if (flag)
	{
		FQ2* pFq = FQ2Open("OFS", sServiceid);
...
/*----------------------------------------------------------------------------*/
{
	...
	if (flag)
	{
		if (flag)
		{
			 ...
			 * 파일명을 만든다.
			 */
			sprintf(sFileName, CfgGetMaster("ib.path.file.bfscom")
				, sizeof(pBat->sSR       ), sizeof(pBat->sSR       ), pBat->sSR
				, sizeof(pBat->sOrgCode  ), sizeof(pBat->sOrgCode  ), pBat->sOrgCode
				, sizeof(pBat->sTrCode   ), sizeof(pBat->sTrCode   ), pBat->sTrCode
				, sizeof(pBat->sDate     ), sizeof(pBat->sDate     ), pBat->sDate
		...
		if (flag)
		{
			...
			{
				LOG(_FL_, 0, 0, "ERROR : (배치)파일이 존재하지 않습니다.[%s]", sFileName);
				return -1;
			}
		}
		...
		if (flag)
		{
			...
			{
				LOG(_FL_, 0, 0, "ERROR : (배치)파일 상태를 구할 수 없습니다.[%s]", sFileName);
				return -1;
			}

			if (iFileSize != fStat.st_size)
			{
				LOG(_FL_, 0, 0, "ERROR : (배치)파일 크기와 (REC x CNT)와 다릅니다.[%s]", sFileName);
				return -1;
			}
		}
	}
