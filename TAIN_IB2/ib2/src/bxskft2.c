...
/*----------------------------------------------------------------------------*/
{
	...
	if (flag)
	{
		...
		if (flag)
		{
			...
			/* 배치파일 정보를 출력 */
			if (flag) LOG(_FL_, 0, 0, "BAT_FILE [%s]", sFileName);
		}
		...
		if (flag)
		{
			...
			if (!flag) LOG(_FL_, 0, 0, "sLine [%s]", sLine);
		}

		if (flag)
		{
			/*
			 * DATE.2013.09.16 by Kang Seok
			 * sFileId 를 얻는다.
			 */
			char str[10];
			sprintf(str, "%4.4s", CfgGetBxaTr("FILEID"));

			if (strcmp(str, "GP20") == 0
			||  strcmp(str, "GP21") == 0
			||  strcmp(str, "GP22") == 0
			||  strcmp(str, "GP27") == 0)
			{
				sprintf(sFileId, "%4.4s%4.4s", str, sLine + 47);
			}
			else if (strcmp(str, "GP23") == 0
			|| strcmp(str, "GP24") == 0
			|| strcmp(str, "GP25") == 0)
			{
				sprintf(sFileId, "%4.4s%4.4s", str, sLine + 85);
			}
			else
			{
				sprintf(sFileId, "%4.4s%4.4s", str, sLine + 23);
			}

			if (flag) LOG(_FL_, 0, 0, "sFileId [%s] sLine [%s]", sFileId, sLine);
		}
	}
...
/*----------------------------------------------------------------------------*/
{
	...
	if (flag)
	{
		...
		while (TRUE)
		{
			...
			if (ret < 0)
			{
				LOG(_FL_, 9, 0, "ERROR : SendBatFileFILE");
				return ret;
			}
			...
			while (TRUE)
			{
				...
				if (!flag) return 0; /* DATE.2013.09.23 : 임시 */
	...
	if (flag)
	{
		...
		while (TRUE)
		{
			...
			if (flag)
			{
				...
				{
					if (flag) LOG(_FL_, 9, 0, "ERROR : CSockWriteRead");
					break;
				}
			}
