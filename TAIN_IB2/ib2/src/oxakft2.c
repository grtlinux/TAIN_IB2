/*
 * PROGRAM ID   : oxatat2.c
 * PROGRAM NAME : oxatat2 관련 모듈
 * AUTHOR       : (주)ICA
 * COMMENT      : oxakft2 관련 모듈
 *                대외기관 연결 모듈 (송수신)
...
/*----------------------------------------------------------------------------*/
{
	...
	if (flag) LOG(_FL_, 5, 0, "[FQ_WRITE] <- [%04d:%s] ", iDataLen, sDataLine);
	...
	if (flag)
	{
		...
		if (flag)
		{
			...
			&&  csi->iPollFlag  == FALSE)
			{
				...
				{
					/* POLL REQ 출력 */
					if (flag) LOG(_FL_, 5, 0, "[POLL_SEND] REQ -> [%d:%s] (%s:%d) (fd:%d)"
